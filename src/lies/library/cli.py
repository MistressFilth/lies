...
The deterministic ingest pipeline (Task 8) lives in
:mod:`lies.library.ingest`; this module is the thin Typer wrapper that
wires it onto the root ``lies`` CLI as the ``ingest`` sub-app. The
legacy ``lies ingest-source`` and the ``--no-llm`` opt-out are
deprecated (see ``lies.cli.ingestion``).
"""
...
__all__ = ("library_app", "ingest_cmd")
