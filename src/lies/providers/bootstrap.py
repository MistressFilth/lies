...
def step_providers(partial: PartialConfig, *, prompt: PromptFn) -> None:
    ...
    while True:
        ...
        envs: list[str] = []
        while True:
            env_name = prompt(f"  api_key_env #{len(envs) + 1} name (blank to finish)", "").strip()
            if not env_name:
                if not envs:
                    print("  ✗ api_key_envs must list ≥1 env var name.")
                    continue
                break
...
def _write_env_file(env_path: os.PathLike[str], partial: PartialConfig) -> None:
    """Capture ``os.environ`` values for every declared entry in each
    provider's ``api_key_envs`` chain into a chmod 600 file. Never writes
    key names that are not currently set in the operator's env, and never
    writes env vars from providers the operator did not declare in this
    wizard session — that would be a silent data egress for unrelated
    shell values like ``OPENAI_API_KEY`` set by other tools.
    """
