...
2. Run `lies sync <collection> --source <source> --name <wiki>`.
   ...
   - `bootstrap_collection(wiki, name, source)` — writes
     `~/.config/lies/<wiki>/collections/<name>.yaml`.
   - `sync_collection(wiki, name)` — scrape → normalize → map →
     write → commit (one git commit per run).
3. Inspect the result: `lies collections list` to confirm the
   YAML exists; `find ~/.local/share/lies/library/collections/<name>`
   to confirm the raw mirrors landed.

## Pitfalls

- `--source` mismatches an existing YAML's `source` field →
  `CollectionMismatch`. Use `lies collections modify --set
