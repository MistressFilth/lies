from lies.orchestrator import _validate_claim_citations_legacy
...
def test_validate_keeps_valid_entries() -> None:
    ...
    kept, dropped = _validate_claim_citations_legacy(entries, citations, answer)
    assert len(kept) == 2
    assert dropped == []
...
def test_validate_drops_claim_not_in_answer() -> None:
    ...
    kept, dropped = _validate_claim_citations_legacy(entries, citations, "actual body")
    assert kept == []
    assert len(dropped) == 1
...
def test_validate_drops_negative_index() -> None:
    ...
    kept, dropped = _validate_claim_citations_legacy(entries, citations, "x")
    assert kept == []
...
def test_validate_empty_input() -> None:
    kept, dropped = _validate_claim_citations_legacy([], [], "")
    assert kept == []
    assert dropped == []
...
def test_validate_drops_empty_claim() -> None:
    ...
    kept, dropped = _validate_claim_citations_legacy(entries, citations, "any body")
    assert kept == []
    assert len(dropped) == 1
