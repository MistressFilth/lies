...
def test_translate_collection_or() -> None:
    intent = StructuredIntent(
        ...
        tag_filter=[], exclude_terms=[], body="hx syntax",
    )
...
def test_translate_excludes() -> None:
    intent = StructuredIntent(
        collection_filter=[], tag_filter=["html"], exclude_terms=["javascript"], body="forms",
    )
    assert translate(intent) == "+tag:html -javascript forms"


def test_translate_noop_passes_body_through() -> None:
    intent = StructuredIntent(collection_filter=[], tag_filter=[], exclude_terms=[], body="hello world")
