...
def test_run_lint_end_to_end_with_linter_categories(orch: Orchestrator) -> None:
    ...
    # The per-op catalog upsert keeps ``catalog.db`` in lockstep with
    # every non-system write (the catalog is the source of truth);
    # ``index.md`` itself is not regenerated by the apply envelope. The
    # ``PageUpdate`` carve-out for ``UpdateIndex`` (the repair agent's
    # index rewrite) is what produces the on-disk index update here.
    # alpha.md got a cross-link to beta via the real ``_merge_append_links``
    # rewrite.
